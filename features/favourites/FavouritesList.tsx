...
export default function FavouritesList({ initialRooms }: { initialRooms: Room[] }) {
  ...
  if (rooms.length === 0) {
    return (
      <p className="text-center mt-40 text-muted-foreground">
        {t("no rooms yet")}
      </p>
    )
  }
  ...
  return (
    <div className="grid grid-cols-1 gap-6 lg:gap-10 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 px-10 xl:px-25">
      {rooms.map((room) => (
