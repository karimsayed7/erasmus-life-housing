...
export default function ApplicationsTable({
  ...
  function handleCancel(roomId: string) {
    startTransition(async () => {
      ...
      await cancelRoomRequest(roomId);
    });
  }
    ...
    <div className="px-5 sm:px-10 lg:px-20 xl:px-40">
      {
        optimisticRooms.length == 0 ? (
          <p className="mt-40 text-lg text-center font-semibold text-gray-400">No Room Applications Yet</p>
        ) : (
          <div className="border-2 rounded-2xl shadow-sm">
        <p className="font-semibold text-[16px] pl-9 py-5">{optimisticRooms.length} {t("request")}</p>
        <Table>
          <TableHeader>
          ...
          <TableBody className="bg-gray-50">
            {optimisticRooms.map((room) => {
              const isOpen = openRows.has(room.id);
              return (
                <Fragment key={room.id}>
                  ...
                  >
                    ...
                    <TableCell>{getLocalized(room.approval_status , locale)}</TableCell>
                  ...
                  {isOpen && (
                    <TableRow key={`${room.id}-details`}>
                        <TableCell colSpan={7} className="p-0">
                            <RowContent room={room} handleCancel={handleCancel} isPending={isPending}/>
                        </TableCell>
                    </TableRow>
                  )}
                </Fragment>
